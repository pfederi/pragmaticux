...
import { getPrincipleByOrder, getPrinciples } from '@/data/principles'
...
export default function PrincipleDetail({ initialPrinciple, order }: PrincipleDetailProps) {
  const { t, locale } = useLanguage()
  const principle = getPrincipleByOrder(order, locale) || initialPrinciple

  const allPrinciples = getPrinciples(locale)
          ...
          <div className="flex-1 w-full sm:w-auto">
            ...
            {principle.why_matters && principle.why_matters.length > 0 && (
              <section className="mb-8 sm:mb-12 md:mb-16 pb-8 sm:pb-12 md:pb-16 pt-8 sm:pt-12 md:pt-16 border-t border-b border-border">
                <h2 className="text-2xl sm:text-3xl font-bold mb-4 sm:mb-6 md:mb-8 text-left leading-tight pb-1 bg-gradient-to-r from-foreground to-primary bg-clip-text text-transparent" id="why-it-matters">{t.principles.whyItMatters}</h2>
            ...
            {principle.examples && principle.examples.length > 0 && (
              <section className="mb-8 sm:mb-12 md:mb-16 pb-8 sm:pb-12 md:pb-16 border-b border-border">
                <h2 className="text-2xl sm:text-3xl font-bold mb-4 sm:mb-6 md:mb-8 text-left leading-tight pb-1 bg-gradient-to-r from-foreground to-primary bg-clip-text text-transparent" id="practical-examples">{t.principles.practicalExamples}</h2>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
            ...
            {principle.instead_of_asking && principle.instead_of_asking.length > 0 && (
              <section className="mb-8 sm:mb-12 md:mb-16">
                <h2 className="text-2xl sm:text-3xl font-bold mb-4 sm:mb-6 md:mb-8 text-left leading-tight pb-1 bg-gradient-to-r from-foreground to-primary bg-clip-text text-transparent" id="key-questions">{t.principles.keyQuestions}</h2>
            ...
            {prevPrinciple ? (
              ...
              >
                ...
                <div className="text-left min-w-0 flex-1">
                  <div className="text-xs sm:text-sm text-muted-foreground">{t.principles.previous}</div>
            ...
            {nextPrinciple ? (
              ...
              >
                <div className="text-right min-w-0 flex-1 sm:flex-initial sm:flex-none">
                  <div className="text-xs sm:text-sm text-muted-foreground">{t.principles.next}</div>
