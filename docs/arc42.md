...
5. **State Persistence**
   - localStorage key: `pragmatic-ux-decision-helper-state`
...
### 5.6 Decision Tree Structure

```mermaid
graph TB
    subgraph "Decision Tree Logic"
        Q1[Question 1: Context<br/>startup/eng_driven/corporate]
        Q2[Question 2: Challenge<br/>efficiency/fast_decisions/etc.]
        Q3[Question 3: Time/Budget<br/>tight/adequate]
        Q4[Question 4: UX Acceptance<br/>low/high]
        
        Rules[Rule Engine<br/>Match IF conditions]
        Results[Results<br/>Principles + Methods]
    end
...
**Decision Tree Rules:**
- Rules match on `if` conditions (context, challenge, time_budget, ux_acceptance)
- Multiple rules can match (combinations)
- Each rule provides `principles` array and `methods` array
- Rules are evaluated in order, all matching rules contribute to results
...
### 6.4 Scenario: Answer Editing in Decision Helper
...
sequenceDiagram
    ...
    User->>Component: View results
    User->>Component: Click "Change" on answer
    Component->>State: Set editing mode
    Component->>Component: Display question with current answer
    User->>Component: Select new answer
    Component->>State: Update answer
    Component->>Storage: Save updated state
    Component->>Rules: Recalculate results
    Rules->>Component: Return new results
    Component->>State: Update results
    Component->>Component: Display updated results
    Component->>Storage: Save updated results
```
...
- Key: `pragmatic-ux-decision-helper-state`
- Structure: `{ currentQuestionIndex, userAnswers, results }`
